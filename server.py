...
#as long as it`s connected it prints out responses of the client in server.
...
def get_msg():
    while True:
        ...
        username = pickle.loads(c.recv(1024)).sender    #recive message string from server
        if username not in bot_username:                #If connection is done successfully
            bot_username.append(username)               #
            ...
            print(f"Bot {username}, is taken please try again with another bot name.")
            c.send(pickle.dumps(connection_bad))
            bot_username.pop(bot_username.index(username))
            c.close()
...
if __name__ == "__main__":
    ...
    get_msg()  # executing the main function of the server
